...
export const registerLoadBalancer =  <T>(name: string, provider: T): void => {
 ...
 */
export const registerPublisher = <T> (name: string, provider: T): void => {
...
const getEndpoint = async <T>(loadBalancer: T): Promise<T> => {
  return loadBalancer;
};
 ...
 * @param cfg
 */
const makeServiceEndpoint = (name: string, middleware: any, loadBalancer: any, logger: Logger, cfg?: any): any => {
  ...
  // handleTimeout
  return async <T>(req: any, options: any): Promise<T> => {
...
const generalFactory = (method: any, transports: any, logger: Logger): any => {
  return async(instance: any): Promise<any> => {
    for (let i = 0; i < transports.length; i += 1) {
      try {
        return await (transports[i].makeEndpoint(method, instance));
      } catch (e) {
